          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            ...
            <aside className="lg:col-span-3 hidden lg:block space-y-10 sticky top-32 h-fit">
              <div className="relative group">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-10 pr-4 focus:outline-none focus:border-primary/50 font-bold text-sm"
                />
              </div>

              {sections.map((section) => (
                <div key={section.title} className="space-y-3">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-primary/60 px-3">{section.title}</h4>
                  <ul className="space-y-1">
                    {section.items.map((item) => (
                      <li key={item}>
                        <button className="w-full text-left px-3 py-2 rounded-lg hover:bg-white/5 text-white/40 hover:text-white transition-all font-bold text-sm flex items-center justify-between group">
                          {item}
            ...
            <div className="lg:col-span-9 space-y-16">
              <section>
                ...
                <p className="text-white/40 text-lg max-w-3xl font-medium leading-relaxed">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {highlights.map((h, i) => (
                  <motion.div
                    ...
                    className="p-6 bg-slate-900/50 border border-white/10 rounded-[24px] hover:border-primary/30 transition-all"
                  >
                    <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-primary mb-4">
                      <h.icon size={20} />
                    </div>
                    <h3 className="text-base font-black uppercase tracking-tight mb-1">{h.title}</h3>
                    <p className="text-xs text-white/40 font-medium leading-relaxed">{h.desc}</p>
                  </motion.div>
                ))}
              </div>
              ...
              <section className="space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-2xl font-black uppercase tracking-tight">Quick <span className="text-primary">Start</span></h2>
                </div>
                ...
                <div className="relative group">
                  ...
                  <div className="relative bg-slate-950 rounded-[24px] p-6 border border-white/10 overflow-hidden">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-white/20">supabase-client.ts</span>
                    </div>
                    <pre className="font-mono text-xs text-emerald-400/80 overflow-x-auto leading-relaxed">
...
const { data: memories } = await supabase
  ...
  .order('created_at', { ascending: false });`}
              ...
              <section className="space-y-6">
                <h2 className="text-2xl font-black uppercase tracking-tight">API <span className="text-primary">Endpoints</span></h2>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={endpoint.path} className="flex items-center gap-4 p-4 bg-white/5 rounded-xl">
                      ...
                      <span className="text-xs text-white/40 ml-auto">{endpoint.desc}</span>
                    </div>
                  ))}
                </div>
              </section>
